...
namespace utils
	...
	namespace _
		...
		class tracker_updater
			...
			public:
				tracker_updater(T* ptr) noexcept : my_address{ptr} {}
				...
				tracker_updater(tracker_updater&& move, T* my_address) noexcept 
					...
					if (tracker) { tracker->tracked = my_address; }
					}
			...
			protected:
				...
				utils::observer_ptr<tracker_t<T>> tracker{nullptr};

				// Need to store my_address in case a tracker_t has to be created later
				T* my_address;

				std::shared_ptr<tracker_t<T>>&& get_tracker() noexcept
					{
					if (tracker) { return {tracker}; }

					auto ret{std::make_shared<tracker_t<T>>(my_address)};
					tracker = *ret;
					return std::move(ret);
					}
			};
	...
	class trackable : private _::tracker_updater<trackable>
		{
	...
	// The wrapper way
	template <typename T>
	class trackable_wrapper : private _::tracker_updater<T>, public wrapper<T>
		{
	...
	class tracking_ptr
		...
		public:
			...
			//trackable via inheritance //TODO make available only if "std::is_base_of<tracked, T>"
			tracking_ptr(T& tracked) : tracker{tracked.get_tracker()} {}
			tracking_ptr& operator=(T& tracked) { tracker = tracked.get_tracker(); return *this; }

			//trackable via wrapper
			tracking_ptr(trackable_wrapper<T>& tracked) : tracker{tracked.get_tracker()} {}
			tracking_ptr& operator=(trackable_wrapper<T>& tracked) { tracker = tracked.get_tracker(); return *this; }
			...
			~tracking_ptr()
				{
				// If the last tracking pointer is destroyed, destroy the tracker as well
				if (tracker.use_count() == 1) { tracker->tracked->tracker = nullptr; }
				}
