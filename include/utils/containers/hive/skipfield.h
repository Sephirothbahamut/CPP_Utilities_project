...
#ifndef PLF_FROM_RANGE // To ensure interoperability with other utils::containers::hive lib containers
...
namespace std
	{
	template <utils::containers::hive::hive_iterator_concept it_type, typename distance_type>
	...
	template <utils::containers::hive::hive_iterator_concept it_type>
...
namespace utils::containers::hive
	...
	class skipfield : private allocator_type // Empty base class optimisation - inheriting allocator functions
		{
		...
		private:
			...
			static constexpr skipfield_type default_min_block_capacity() noexcept
				{
				constexpr skipfield_type adaptive_size = static_cast<skipfield_type>(((sizeof(skipfield) + sizeof(group)) * 2) / sizeof(aligned_element_type));
			...
			void blank() noexcept
				{
				if constexpr (std::is_standard_layout<skipfield>::value && std::allocator_traits<allocator_type>::is_always_equal::value && std::is_trivial<group_pointer_type>::value && std::is_trivial<aligned_pointer_type>::value && std::is_trivial<skipfield_pointer_type>::value)
					{ // If all pointer types are trivial, we can just nuke the member variables from orbit with memset (nullptr is always 0):
					std::memset(static_cast<void*>(&end_iterator), 0, offsetof(skipfield, min_block_capacity));
					}
				else
					{
		...
		public:
			...
			explicit skipfield(const allocator_type& alloc) noexcept :
			...
			constexpr skipfield() noexcept(noexcept(allocator_type())) :
				skipfield(allocator_type())
				{}



			skipfield(const hive_limits capacities, const allocator_type& alloc) noexcept :
				allocator_type(alloc),
				erasure_groups_head(nullptr),
				unused_groups_head(nullptr),
				total_size(0),
			...
			explicit skipfield(const hive_limits capacities) noexcept(noexcept(allocator_type())) :
				skipfield(capacities, allocator_type())
				{}
			...
			skipfield(const skipfield& source, const std::type_identity_t<allocator_type>& alloc) :
				allocator_type(alloc),
				erasure_groups_head(nullptr),
				unused_groups_head(nullptr),
				total_size(0),
			...
			skipfield(const skipfield& source) :
				skipfield(source, std::allocator_traits<allocator_type>::select_on_container_copy_construction(source))
				{}
			...
			skipfield(skipfield&& source, const std::type_identity_t<allocator_type>& alloc) :
				allocator_type(alloc),
			...
			skipfield(skipfield&& source) noexcept :
				end_iterator(std::move(source.end_iterator)),
				begin_iterator(std::move(source.begin_iterator)),
				erasure_groups_head(std::move(source.erasure_groups_head)),
				unused_groups_head(std::move(source.unused_groups_head)),
			...
			skipfield(const size_type fill_number, const element_type& element, const hive_limits block_limits, const allocator_type& alloc = allocator_type()) :
				allocator_type(alloc),
				erasure_groups_head(nullptr),
				unused_groups_head(nullptr),
				total_size(0),
			...
			skipfield(const size_type fill_number, const element_type& element, const allocator_type& alloc = allocator_type()) :
				skipfield(fill_number, element, default_block_capacity_limits(), alloc)
				{}
			...
			skipfield(const size_type fill_number, const hive_limits block_limits, const allocator_type& alloc = allocator_type()) :
				allocator_type(alloc),
				erasure_groups_head(nullptr),
				unused_groups_head(nullptr),
				total_size(0),
			...
			skipfield(const size_type fill_number, const allocator_type& alloc = allocator_type()) :
				skipfield(fill_number, default_block_capacity_limits(), alloc)
				{}
			...
			skipfield(const typename std::enable_if_t<!std::numeric_limits<iterator_type>::is_integer, iterator_type>& first, const iterator_type& last, const hive_limits block_limits, const allocator_type& alloc = allocator_type()) :
				allocator_type(alloc),
				erasure_groups_head(nullptr),
				unused_groups_head(nullptr),
				total_size(0),
			...
			template<typename iterator_type>
			skipfield(const typename std::enable_if_t<!std::numeric_limits<iterator_type>::is_integer, iterator_type>& first, const iterator_type& last, const allocator_type& alloc = allocator_type()) :
				skipfield(first, last, default_block_capacity_limits(), alloc)
				{}
			...
			skipfield(const std::initializer_list<element_type>& element_list, const hive_limits block_limits, const allocator_type& alloc = allocator_type()) :
				allocator_type(alloc),
				erasure_groups_head(nullptr),
				unused_groups_head(nullptr),
				total_size(0),
			...
			skipfield(const std::initializer_list<element_type>& element_list, const allocator_type& alloc = allocator_type()) :
				skipfield(element_list, default_block_capacity_limits(), alloc)
				{}
			...
			skipfield(utils::containers::hive::ranges::from_range_t, range_type&& rg, const hive_limits block_limits, const allocator_type& alloc = allocator_type()) :
				allocator_type(alloc),
				erasure_groups_head(nullptr),
				unused_groups_head(nullptr),
				total_size(0),
			...
			template<class range_type>
				requires std::ranges::range<range_type>
			skipfield(utils::containers::hive::ranges::from_range_t, range_type&& rg, const allocator_type& alloc = allocator_type()) :
				skipfield(utils::containers::hive::ranges::from_range, std::move(rg), default_block_capacity_limits(), alloc)
				{}
		...
		private:
			...
			void consolidate()
				...
				if constexpr (std::is_nothrow_move_constructible<element_type>::value && std::is_nothrow_move_assignable<element_type>::value)
					{
					skipfield temp(hive_limits(min_block_capacity, max_block_capacity));
		...
		public:
			...
			skipfield& operator = (const skipfield& source)
				{
				assert(&source != this);
			...
			// Move assignment
			skipfield& operator = (skipfield&& source) noexcept(std::allocator_traits<allocator_type>::propagate_on_container_move_assignment::value || std::allocator_traits<allocator_type>::is_always_equal::value)
				{
				assert(&source != this);
				destroy_all_data();
				...
				if (std::allocator_traits<allocator_type>::propagate_on_container_move_assignment::value || std::allocator_traits<allocator_type>::is_always_equal::value || static_cast<allocator_type&>(*this) == static_cast<allocator_type&>(source))
					...
					if constexpr ((std::is_trivially_copyable<allocator_type>::value || std::allocator_traits<allocator_type>::is_always_equal::value) &&
						...
						{
						std::memcpy(static_cast<void*>(this), &source, sizeof(skipfield));
						}
					else
						{
						end_iterator = std::move(source.end_iterator);
			...
			skipfield& operator = (const std::initializer_list<element_type>& element_list)
				{
				range_assign(element_list.begin(), static_cast<size_type>(element_list.size()));
				return *this;
				}
		...
		public:
			...
			void swap(skipfield& source) noexcept(std::allocator_traits<allocator_type>::propagate_on_container_swap::value || std::allocator_traits<allocator_type>::is_always_equal::value)
				{
				assert(&source != this);
				...
				if constexpr (std::allocator_traits<allocator_type>::is_always_equal::value && std::is_trivial<group_pointer_type>::value && std::is_trivial<aligned_pointer_type>::value && std::is_trivial<skipfield_pointer_type>::value) // if all pointer types are trivial we can just copy using memcpy - avoids constructors/destructors etc and is faster
					{
					char temp[sizeof(skipfield)];
					std::memcpy(&temp, static_cast<void*>(this), sizeof(skipfield));
					std::memcpy(static_cast<void*>(this), static_cast<void*>(&source), sizeof(skipfield));
					std::memcpy(static_cast<void*>(&source), &temp, sizeof(skipfield));
					}
					...
					{
					skipfield temp(std::move(source));
			...
			class hive_iterator
				...
				public:
					...
					typedef typename skipfield::value_type 			value_type;
					typedef typename skipfield::difference_type		difference_type;
					typedef hive_reverse_iterator<is_const> 	reverse_type;
					typedef typename std::conditional_t<is_const, typename skipfield::const_pointer, typename skipfield::pointer>		pointer;
					typedef typename std::conditional_t<is_const, typename skipfield::const_reference, typename skipfield::reference>	reference;
			...
			class hive_reverse_iterator
				...
				public:
					...
					typedef typename skipfield::value_type 		value_type;
					typedef typename skipfield::difference_type	difference_type;
					typedef typename std::conditional_t<is_const_r, typename skipfield::const_pointer, typename skipfield::pointer>			pointer;
					typedef typename std::conditional_t<is_const_r, typename skipfield::const_reference, typename skipfield::reference>	reference;
					...
					hive_reverse_iterator& operator ++ ()
						{
						skipfield::group_pointer_type& group_pointer = current.group_pointer;
						skipfield::aligned_pointer_type& element_pointer = current.element_pointer;
						skipfield::skipfield_pointer_type& skipfield_pointer = current.skipfield_pointer;
	...
	} // utils::containers::hive namespace
...
namespace std
	...
	void swap(utils::containers::hive::skipfield<element_type, allocator_type>& a, utils::containers::hive::skipfield<element_type, allocator_type>& b) noexcept(std::allocator_traits<allocator_type>::propagate_on_container_swap::value || std::allocator_traits<allocator_type>::is_always_equal::value)
		{
		a.swap(b);
		}
	...
	typename utils::containers::hive::skipfield<element_type, allocator_type>::size_type erase_if(utils::containers::hive::skipfield<element_type, allocator_type>& container, predicate_function predicate)
		{
		typedef typename utils::containers::hive::skipfield<element_type, allocator_type> hive;
	...
	template <class element_type, class allocator_type>
	typename utils::containers::hive::skipfield<element_type, allocator_type>::size_type erase(utils::containers::hive::skipfield<element_type, allocator_type>& container, const element_type& value)
		{
		return erase_if(container, utils::containers::hive::hive_eq_to<element_type>(value));
		}
