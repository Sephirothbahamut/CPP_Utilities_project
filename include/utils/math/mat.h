...
#pragma region constructors
			...
			utils_cuda_available constexpr mat() noexcept : mat{value_type{0}} {}
			...
			utils_cuda_available constexpr mat(Args&... args) noexcept : details::vec_named<T, size>{.array{args...}} {}

			template <std::convertible_to<value_type>... Args>
				requires(sizeof...(Args) >= static_size)
			utils_cuda_available constexpr mat(const Args&... args) noexcept : details::vec_named<T, size>{.array{static_cast<value_type>(args)...}} {}
			...
			utils_cuda_available constexpr mat(      other_t& other) noexcept : details::vec_named<T, size>{.array{std::apply([](      auto&... values) -> std::array<value_type, size> { return {                        values ...}; }, other.array)}} {}

			template <concepts::mat other_t>
				requires(std::convertible_to<typename other_t::value_type, value_type> && other_t::static_size == static_size)
			utils_cuda_available constexpr mat(const other_t& other) noexcept : details::vec_named<T, size>{.array{std::apply([](const auto&... values) -> std::array<value_type, size> { return {static_cast<value_type>(values)...}; }, other.array)}} {}
			...
			utils_cuda_available constexpr mat(const other_t& other, value_type default_value = value_type{0}) noexcept
				{
				size_t i{0};
