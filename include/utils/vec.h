...
namespace utils
	...
	namespace concepts
		...
		concept vec_compatible = vec<other> && std::derived_from<T, utils::vec<typename other::value_type, other::size>>;
		}
...
// 
	...
	template <concepts::vec vec_t, concepts::vec_compatible<vec_t> oth_t> vec_t  operator+ (const vec_t& a, const oth_t& b) noexcept { vec_t ret{}; for (size_t i{0}; i < vec_t::size; i++) { ret[i] = a[i] + b[i]; }; return ret; }
	template <concepts::vec vec_t, concepts::vec_compatible<vec_t> oth_t> vec_t  operator- (const vec_t& a, const oth_t& b) noexcept { vec_t ret{}; for (size_t i{0}; i < vec_t::size; i++) { ret[i] = a[i] - b[i]; }; return ret; }
	template <concepts::vec vec_t, concepts::vec_compatible<vec_t> oth_t> vec_t  operator* (const vec_t& a, const oth_t& b) noexcept { vec_t ret{}; for (size_t i{0}; i < vec_t::size; i++) { ret[i] = a[i] * b[i]; }; return ret; }
	template <concepts::vec vec_t, concepts::vec_compatible<vec_t> oth_t> vec_t  operator/ (const vec_t& a, const oth_t& b) noexcept { vec_t ret{}; for (size_t i{0}; i < vec_t::size; i++) { ret[i] = a[i] / b[i]; }; return ret; }
	
	template <concepts::vec vec_t, concepts::vec_compatible<vec_t> oth_t> vec_t& operator+=(      vec_t& a, const oth_t& b) noexcept { return a = a + b; }
	template <concepts::vec vec_t, concepts::vec_compatible<vec_t> oth_t> vec_t& operator-=(      vec_t& a, const oth_t& b) noexcept { return a = a - b; }
	template <concepts::vec vec_t, concepts::vec_compatible<vec_t> oth_t> vec_t& operator*=(      vec_t& a, const oth_t& b) noexcept { return a = a * b; }
	template <concepts::vec vec_t, concepts::vec_compatible<vec_t> oth_t> vec_t& operator/=(      vec_t& a, const oth_t& b) noexcept { return a = a / b; }
	...
	namespace operators
		...
		inline constexpr struct _dot
			...
			class _inner
				{
				public:
					template <concepts::vec_compatible<vec_t> oth_t>
					vec_t::value_type operator>(const oth_t& rhs) const noexcept { return vec_t::dot(lhs, rhs); }
		...
		inline constexpr struct _cross
			...
			class _inner
				...
				public:
					template <concepts::vec_compatible<vec_t> oth_t>
					vec_t operator>(const oth_t& rhs) const noexcept { return lhs * rhs; }
					_inner(vec_t lhs) noexcept : lhs{lhs} {}
				private:
					const vec_t& lhs;
				};
...
#pragma endregion operators
	...
	template <concepts::vec vec_t, concepts::vec_compatible<vec_t> oth_t>
	inline vec_t lerp(const vec_t& a, const oth_t& b, float t)
		{
		vec_t ret;
	...
	template <concepts::vec vec_t, concepts::vec_compatible<vec_t> min_t, concepts::vec_compatible<vec_t> max_t>
	inline vec_t clamp(const vec_t& in, const min_t& min, const max_t& max) { vec_t ret; for (size_t i{0}; i < vec_t::size; i++) { ret[i] = utils::clamp(in[i], min[i], max[i]); } return ret; }

	template <concepts::vec vec_t, concepts::vec_compatible<vec_t> oth_t>
	inline vec_t min(const vec_t& a, const oth_t& b) { vec_t ret; for (size_t i{0}; i < vec_t::size; i++) { ret[i] = utils::min(a[i], b._data[i]); } return ret; }

	template <concepts::vec vec_t, concepts::vec_compatible<vec_t> oth_t>
	inline vec_t max(const vec_t& a, const oth_t& b) { vec_t ret; for (size_t i{0}; i < vec_t::size; i++) { ret[i] = utils::max(a[i], b._data[i]); } return ret; }
	}